...
55. As a study-document author, I want quick-insert and front-menu labels for outline items in `en.json` for this contribution, so that English UI is complete (community i18n deferred per CONTRIBUTING).
...
- v1 contribution: **`en.json` only** per CONTRIBUTING (community translation unavailable).
...
- Community i18n beyond `en.json` for initial PR.
